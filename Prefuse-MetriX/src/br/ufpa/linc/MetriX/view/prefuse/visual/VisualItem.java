 ...
 * is associated with one and only one {@link Visualization}.</p>
 * 
 ...
 * a {@link Renderer}, which contains instructions for drawing
 * the item. The Renderer to use for a given item is decided by the
 * {@link RendererFactory} associated with the item's
 * backing Visualization.</p>
 * 
 * <p>Finally, actually setting the visual properties of VisualItems is
 * commonly done by the many {@link Action} modules available
...
public interface VisualItem extends Tuple {
     ...
     * {@link ColorAction} might assign colors
     * based on the flag).
     ...
     * included in the {@link Constants} class. This value only has an
     * effect if a Renderer that supports different shapes is used
     * (e.g., {@link ShapeRenderer}.
