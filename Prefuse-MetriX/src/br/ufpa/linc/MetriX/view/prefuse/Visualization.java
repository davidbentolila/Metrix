package br.ufpa.linc.MetriX.view.prefuse;
...
import br.ufpa.linc.MetriX.view.prefuse.activity.Activity;
import br.ufpa.linc.MetriX.view.prefuse.activity.ActivityManager;
import br.ufpa.linc.MetriX.view.prefuse.activity.ActivityMap;
import br.ufpa.linc.MetriX.view.prefuse.controls.FocusControl;
...
import br.ufpa.linc.MetriX.view.prefuse.data.tuple.CompositeTupleSet;
...
import br.ufpa.linc.MetriX.view.prefuse.render.RendererFactory;
import br.ufpa.linc.MetriX.view.prefuse.util.PrefuseConfig;
import br.ufpa.linc.MetriX.view.prefuse.util.PrefuseLib;
import br.ufpa.linc.MetriX.view.prefuse.util.collections.CompositeIterator;
...
import br.ufpa.linc.MetriX.view.prefuse.visual.VisualGraph;
...
import br.ufpa.linc.MetriX.view.prefuse.visual.VisualTupleSet;
import br.ufpa.linc.MetriX.view.prefuse.visual.expression.InGroupPredicate;
import br.ufpa.linc.MetriX.view.prefuse.visual.expression.ValidatedPredicate;
import br.ufpa.linc.MetriX.view.prefuse.visual.expression.VisiblePredicate;
import br.ufpa.linc.MetriX.view.prefuse.visual.tuple.TableDecoratorItem;
import br.ufpa.linc.MetriX.view.prefuse.visual.tuple.TableEdgeItem;
import br.ufpa.linc.MetriX.view.prefuse.visual.tuple.TableNodeItem;
 ...
 * structure (i.e., {@link Table}, {@link Graph},
 * or {@link Tree}), this class takes source data such as that
 * loaded from a file (see {@link br.ufpa.linc.MetriX.view.prefuse.data.io}) or from a relational
 * database (see {@link br.ufpa.linc.MetriX.view.prefuse.data.io.sql}) and creates a visual
 ...
 * {@link Table}, {@link Graph}, or
 * or {@link Tree}, a corresponding instance of
 * {@link VisualTable}, {@link VisualGraph}, or
 * {@link VisualTree} is created and stored in the
 ...
 * visual abstraction. Similarly, all {@link Tuple},
 * {@link Node}, or {@link Edge}
 * instances used to represent an entry in the source data have a
 * corresponding {@link VisualItem},
 * {@link NodeItem}, or {@link EdgeItem}
 ...
 * quite useful when crafting {@link Action} instances that
 ...
 * {@link Predicate} to determine the items to
 ...
 * {@link DecoratorItem} which "decorates" another pre-existing
 * VisualItem with a separate interactive visual object, and
 * {@link AggregateItem} which provides an interactive visual
 ...
 * represented using {@link DefaultTupleSet} instances.
 * Also, some of the interactive controls provided by the
 * {@link br.ufpa.linc.MetriX.view.prefuse.controls} package populate these sets by default. See
 * {@link FocusControl} for an example.</p>
 * 
 ...
 * filtering {@link Predicate} can be used to
 ...
 * {@link InGroupPredicate}). The Visualization's
 ...
 * {@link Action} instances that can be invoked to perform
 ...
 * Action's {@link Action#run(double)} method. Processing
 ...
 * {@link br.ufpa.linc.MetriX.view.prefuse.action} packages and the top-level
 * {@link Action} class.</p>
 * 
...
public class Visualization {
     ...
     * @return a visual abstraction of the input data, a {@link VisualTupleSet}
     * instance
     */
    ...
    /**
     * Add an empty {@link VisualTable} to this visualization, using the given data
     ...
     * @param group the data group name for the visualized data
     * @return the added {@link VisualTable}
     */
     ...
     * @param group the data group name for the visualized data
     * @param table the data table to visualize
     * @param schema the data schema to use for the created {@link VisualTable}
     */
    public synchronized VisualTable addTable(
     ...
     * @param group the data group name for the visualized data
     * @param table the data table to visualize
     * @param filter a filter {@link Predicate} determining which data Tuples in the
     * input table are visualized
     * @param schema the data schema to use for the created {@link VisualTable}
     */
    public synchronized VisualTable addTable(
    ...
    /**
     * Add a {@link VisualTable} to this visualization, using the table's
     ...
     * implementations of VisualTable into a {@link Visualization}. It is intended
     ...
     * realizeed as {@link DecoratorItem} instances that provide
     ...
     * @param schema schema indicating which variables should <b>not</b> be
     * inherited from the source data group and instead be managed locally
     * by the generated {@link VisualTable}
     * @return the generated {@link VisualTable} of DecoratorItem instances
     */
    public synchronized VisualTable addDecorators(
    ...
    /**
     * Get the {@link VisualItem} associated with a source data tuple, if it exists.
     ...
     * null if no such {@link VisualItem} exists
     */
    ...
    /**
     * Get the {@link TupleSet} associated with the given data group name. 
     * @param group a visual data group name
     * @return the data group {@link TupleSet}
     */
    ...
    /**
     * Indicates if a given {@link VisualItem} is contained in the given visual
     * data group.
     * @param item the {@link VisualItem} instance
     ...
     * @param group the name of the focus group to add
     * @param tset the {@link TupleSet} for the focus group
     */
    ...
    /**
     * Invalidate the bounds of all {@link VisualItem}s in the given group. This
    ...
    /**
     * Invalidate the bounds of all {@link VisualItem}s in this visualization. This
     * will cause the bounds to be recomputed for all items upon the next
     * redraw.
     */
     ...
     * {@link ExpressionParser} class. If a
     ...
     * the iteration.
     * @return a filtered iterator over {@link VisualItem}s
     */
    ...
    /**
     * Add a data processing {@link Action} to this {@link Visualization}. The {@link Action} will be
     * updated to use this {@link Visualization} in its data processing.
    ...
    /**
     * Get the data processing {@link Action} with the given name.
     * @param name the name of the Action
     * @return the requested {@link Action}, or null if the name was not found
     */
    ...
    /**
     * Remove a data processing {@link Action} registered with this visualization.
     ...
     * @param name the name of the Action
     * @return the removed {@link Action}, or null if no action was found
     */
    ...
    /**
     * Schedule the {@link Action} with the given name to run immediately. The running
     * of all {@link Action}s is managed by the
     * {@link ActivityManager}, which runs in a dedicated
     * thread.
     * @param action the name of the Action to run
     * @return the {@link Action} scheduled to run
     */
    ...
    /**
     * Schedule the {@link Action} with the given name to run at the specified
    ...
    /**
     * Schedule the {@link Action} with the given name to run after another Action
     ...
     * managed by the {@link ActivityManager}, which runs
    ...
    /**
     * Schedule the {@link Action} with the given name to always run after another Action
    ...
    /**
     * Cancel the {@link Action} with the given name, if it has been scheduled.
    ...
    /**
     * Set the {@link RendererFactory} used by this Visualization. The {@link RendererFactory}
     * is responsible for providing the Renderer instances used to draw
     * the VisualItems.
     * @param rf the {@link RendererFactory} to use.
     */
    ...
    /**
     * Get the {@link RendererFactory} used by this {@link Visualization}.
     * @return this {@link Visualization}'s {@link RendererFactory}
     */
     ...
     * {@link RendererFactory} and returns the result.
     * @param item the item to retreive the renderer for
     * @return the {@link Renderer} for drawing the
     * given item
     */
     ...
     * {@link Display#setVisualization(Visualization)} method.
     ...
     * @return the {@link Display} at the given index
     */
